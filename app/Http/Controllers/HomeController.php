...
class HomeController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $datalingkar = DB::table('pasien')
        ->join('datalingkar','datalingkar.pasien_id','pasien.id')
        ->join('tinggibadan','tinggibadan.pasien_id','pasien.id')
        ->join('beratbadan','beratbadan.pasien_id','pasien.id')
        ->paginate(6);
        
        $lingkar = pasien::where('name','like',"%".$search."%")
                    ->join('datalingkar','datalingkar.pasien_id','pasien.id')
                    ->join('tinggibadan','tinggibadan.pasien_id','pasien.id')
                    ->join('beratbadan','beratbadan.pasien_id','pasien.id');
        $xAxis = [];
